...
# 
# P.S.: хоть такой подход и вызывает "зависание" основного потока выполнения
# кода программы, на практики он очень даже имеет место быть. Все эти операции
# перебора независимы друг от друга, что позволяет с относительной легкостью
# выполнять их в разных потоках. Единственное, что нужно учитывать при работе
# с много поточностью это доступ к глабальным переменным, в данном случае к
# словарю best, нужно согласовать его изменение разными потоками, чтобы там
# оказалось действительно оптимальное значение. Хоть это и не сверхсложно, но
# я почти уверен, это в задании не предполагалось, так что не буду на это
# тратить время второй пункт не за горами.
